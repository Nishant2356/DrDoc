...
export const piiRedactionPrompt = new SystemMessage(`
    ...
    Your job is to process the provided raw medical consultation transcript and accomplish two tasks:
    1. Extract the patient's personal information (name, age, gender, contact, etc.).
    2. Replace Personally Identifiable Information (PII) in the transcript with generic placeholders like [PATIENT_NAME], [LOCATION], [DATE], or [CONTACT].
    ...
    3. Output ONLY a valid JSON object with no markdown formatting. It must follow this exact structure:
    {
      "patientInfo": {
      ...
      "redactedTranscript": "the full transcript with PII replaced by placeholders"
    }
`);
