...
const graphState = {
  ...
  patientInfo: { value: (x: any, y: any) => y ?? x, default: () => ({}) },
...
const knowledgeRetrievalNode = async (state: DrDocState) => {
  console.log("🟢 [Node] Fetching RAG Doctor Preferences (DB)...");

  try {
    const { prisma } = await import("../db");
    // Using a hardcoded generic retrieval for now, but you would normally pass the user ID in the state
    // To simplify, we get the first doctor with a profile, or default to empty
    const firstDoc = await prisma.user.findFirst({
      where: { role: "DOCTOR", clinicalProfile: { isNot: null } },
      include: { clinicalProfile: true }
    });
    console.log(firstDoc)
    if (firstDoc?.clinicalProfile) {
      const profile = `Specialties: ${firstDoc.clinicalProfile.specialties.join(", ")}\nCommon Medicines: ${firstDoc.clinicalProfile.commonMedicines.join(", ")}\nPreferences: ${firstDoc.clinicalProfile.notePreferences}`;
      return { doctorPreferences: profile };
    }
  } catch (e) {
    console.error("DB Fetch error in agent", e);
  }

  return { doctorPreferences: "No preferences found." };
};
...
// Execute the test function removed since this is now a library module
