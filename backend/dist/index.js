...
const graphState = {
    originalTranscript: { value: (x, y) => y ?? x, default: () => "" },
    redactedTranscript: { value: (x, y) => y ?? x, default: () => "" },
    doctorPreferences: { value: (x, y) => y ?? x, default: () => "" },
    draftNote: { value: (x, y) => y ?? x, default: () => "" },
    guardrailStatus: { value: (x, y) => y ?? x, default: () => "PENDING" },
    guardrailError: { value: (x, y) => y ?? x, default: () => "" },
    humanReviewStatus: { value: (x, y) => y ?? x, default: () => "PENDING" },
    humanFeedbackText: { value: (x, y) => y ?? x, default: () => "" },
    rootCause: { value: (x, y) => y ?? x, default: () => null },
};
// ==========================================
// 2. NODE FUNCTIONS (The AI Actions)
// ==========================================
const piiRedactionNode = async (state) => {
    ...
    try {
        ...
        return { redactedTranscript: response.content };
    }
    catch (error) {
...
const knowledgeRetrievalNode = async (state) => {
...
const generateNoteNode = async (state) => {
    ...
    try {
        ...
        return { draftNote: response.content };
    }
    catch (error) {
...
const safetyGuardrailNode = async (state) => {
...
const autoCorrectorNode = async (state) => {
...
const humanHandoffNode = async (state) => {
...
;
const feedbackIntegratorNode = async (state) => {
    ...
    try {
        ...
        const response = await llm.invoke([systemPrompt, userPrompt]);
        const structuredInstructions = response.content;
...
const rootCauseAnalyzerNode = async (state) => {
    ...
    try {
        ...
        const response = await llm.invoke([systemPrompt, userPrompt]);
        // 5. Parse the JSON
        const cleanJsonText = response.content.replace(/```json/g, '').replace(/```/g, '').trim();
...
const updateDatabaseNode = async (state) => {
    ...
    try {
        ...
        const response = await llm.invoke([systemPrompt, userPrompt]);
        const updatedDatabaseString = response.content;
...
const workflow = new StateGraph({ channels: graphState })
    ...
    .addConditionalEdges("human_handoff", checkHumanDecision, 
